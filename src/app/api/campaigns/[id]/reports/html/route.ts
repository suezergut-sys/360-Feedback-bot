...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .vr-no-feedback {
    ...
    border: 1px dashed #e2e8f0; border-radius: 8px; margin-bottom: 32px;
  }
...
<div class="vr-page">
  ...
  ${!hasFeedback ? `<div class="vr-no-feedback">Данные обратной связи ещё не получены. Дождитесь завершения интервью и генерации отчётов.</div>` : `
  <!-- Feedback result bars -->
  <div class="vr-section">
    <div class="vr-section-title">Результаты обратной связи</div>
    <div class="vr-legend">
      <span><span class="vr-legend-dot" style="background:#f97316"></span>Зона для развития</span>
      <span><span class="vr-legend-dot" style="background:#22c55e"></span>Сильная сторона</span>
    </div>
  ...
  <!-- Comments -->
  <div class="vr-section">
    <div class="vr-section-title">Комментарии к обратной связи</div>
    ${data.competencyGroups.map((group) => `
      <div class="vr-group-title">${esc(group.groupName)}</div>
      ${group.competencies.map((comp) => renderCompetencyComments(comp)).join("")}
    `).join("")}
  </div>
  `}
...
function renderGroupMatrix(
  ...
  return `<table class="vr-matrix">
    <thead>
      <tr>
        <th>Компетенция</th>
        ${ALL_ROLES.map((role) => `<th class="role-header">${esc(ROLE_LABELS[role])}<br><span style="font-weight:400;font-size:10px">${roleCounts[role]} чел.</span></th>`).join("")}
      </tr>
    </thead>
    <tbody>
...
function renderCompetencyComments(comp: VisualCompetencyData): string {
  const hasAnyStrength = comp.comments.strengths.length > 0;
  const hasAnyGrowth = comp.comments.growth.length > 0;

  return `<div class="vr-comp-section">
    <div class="vr-comp-section-title">${esc(comp.name)}</div>
    ${comp.behavioralMarkers.length > 0 ? `<div class="vr-markers">${comp.behavioralMarkers.map((m) => `<div class="vr-marker-item">${esc(m)}</div>`).join("")}</div>` : ""}
    ${hasAnyStrength ? `<div class="vr-comment-block">
      <div class="vr-comment-type str">СИЛЬНАЯ СТОРОНА</div>
      ${comp.comments.strengths.map((s) => `<div class="vr-comment-quote">«${esc(s)}»</div>`).join("")}
    </div>` : ""}
    ${hasAnyGrowth ? `<div class="vr-comment-block">
      <div class="vr-comment-type dev">ЗОНА ДЛЯ РАЗВИТИЯ</div>
      ${comp.comments.growth.map((g) => `<div class="vr-comment-quote">«${esc(g)}»</div>`).join("")}
    </div>` : ""}
    ${!hasAnyStrength && !hasAnyGrowth ? '<div class="vr-no-data">Комментарии не получены.</div>' : ""}
  </div>`;
}
