...
const AI_ANALYSIS_SYSTEM_PROMPT = [
  ...
  "Твоя задача — проанализировать результаты оценки 360° и написать персональный анализ напрямую для оцениваемого сотрудника.",
  "Обращайся к сотруднику на «ты»: «тебе стоит обратить внимание», «тебе рекомендуется», «ты проявляешь», «твои коллеги отмечают» и т.д.",
  "Тон — доброжелательный, конкретный, поддерживающий. Без канцелярита и общих фраз.",
...
export async function generateAndSaveAiAnalysis(campaignId: string): Promise<void> {
  ...
  const userPrompt = [
    ...
    "",
    "Напиши анализ строго по трём разделам, обращаясь напрямую к сотруднику на «ты»:",
    "### На что тебе стоит обратить внимание",
    "(выдели ключевые сигналы из данных: расхождения между самооценкой и оценкой других, низкие оценки, повторяющиеся темы в открытых ответах — сформулируй конкретно и по-человечески)",
    "### Что стоит прояснить дополнительно",
    "(укажи, какие аспекты требуют уточнения, и предложи конкретные инструменты: встречи 1-on-1, наблюдение, коучинг, дополнительный опрос и т.д. — обращайся на «ты»)",
    "### Компетенции, которые тебе стоит развивать",
    "(перечисли компетенции в порядке приоритета, кратко объясни почему — опираясь на данные; используй формулировки «тебе рекомендуется», «обрати внимание на», «твои результаты показывают»)",
  ].join("\n");
